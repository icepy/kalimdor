	...
	*/
	...
	var _init = {
		...
		_create:function(){
			...
			$d.wrap('<div class="kalimdor_father_com" style="width:'+_config.width+'px;height:'+(_config.height+68)+'px;position: absolute;left:'+_bdW+'px;top:'+_bdH+'px;display:none;z-index:'+_config.zIndex+'"></div>');
			module.parent = $d.parent();
			if(_config.cursor !== ''){
				module.parent.prepend('<div class="kalimdor_top_title" style="width:'+(_config.width-5)+'px;cursor:'+_config.cursor+'" title="'+_config.title+'">'+_config.title+'</div>');
			}else{
				module.parent.prepend('<div class="kalimdor_top_title" style="width:'+(_config.width-5)+'px;" title="'+_config.title+'">'+_config.title+'</div>');
			}
			module.parent.append('<div class="kalimdor_bottom_but" style="width:'+_config.width+'px;"></div>');
			...
			if(_config.buttons.length !== 0){
				...
				$.each(_config.buttons,function(i){
					_butStr += '<button class="kalimdor_bottom_items" title="'+_config.buttons[i].text+'">'+_config.buttons[i].text+'</button>';
				});
		...
		_shade:function(){
			if(_config.shade){
				$('body').append('<div class="kalimdor_dialog_shade" style="z-index:'+(_config.zIndex - 10)+';top:0px;left:0px;"></div>');
			}
			module.shade = $('.kalimdor_dialog_shade');
		},
		//关闭遮罩
		_openEsc:function(){

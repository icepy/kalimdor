...
*	搜索自定义选择
*/
kalimdor.addplug('searchSelect',function(d,o){
	var $d = $('#'+d);
	/*
	*	@config 配置参数
	*	@width 设置宽
	...
	*/
	...
	var _init = {
		...
		_create:function(){
			var that = this;
			$d.wrap('<div class="kalimdor_search" style="width:'+_config.width+'px;"></div>');
			$d.addClass('kalimdor_select_but');
			$d.css({width:_config.width,height:_config.height});
			$d.append(_config.title);
			module.parent = $d.parent();
			var _dom = ''
			_dom +='<ul class="kalimdor_select_content" style="display:none;" s="n">';
